...
# 训练函数
def train(model, device, train_loader, optimizer, criterion, epoch, profiler=None):
    model.train()
    train_loss = 0
    correct = 0
    total = 0
...
def test(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            with record_function("test_data_to_device"):
                data, target = data.to(device), target.to(device)
            
            with record_function("test_forward"):
                output = model(data)
...
def main():
    ...
    with profile(
        ...
        profile_memory=True,
        with_stack=True,
        schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=6)
    ) as prof:
        for epoch in range(1, epochs + 1):
            print(f"\n第 {epoch} 轮训练")
            train_loss, train_acc = train(model, device, trainloader, optimizer, criterion, epoch, prof)
    ...
    # 导出profiler日志
    with open('profiler_log.txt', 'w') as f:
    ...
    print("Profiler日志已保存到 profiler_log.txt")
