...
def dynamic_threshold(score: np.ndarray, ratio: float = 3):
    ...
    ---
    ### Parameters:
        score: np.ndarray 异常评分，可以是1维数组(单列)或2维数组(多列)
        ratio: float (default = 3) 标准差比例，例：threshold = mean + `ratio` * std
    ### Returns:
        proba: 预测异常，与输入维度相同的数组，1表示异常，0表示正常
    '''
    std = np.std(score)
    mean = np.mean(score)
    ...
    if threshold >= 1:
        ...
        threshold = 0.001
    proba = np.array(score>threshold, dtype=np.int32)
    # proba = np.array(proba, dtype=int)
    return proba
    # 判断输入维度
    # if score.ndim == 1:
    ...
    #     # 生成预测结果
    #     proba = np.array(score > threshold, dtype=int)
        
    # else:
    #     # 多列数据处理
    #     n_cols = score.shape[1]
    #     proba = np.zeros_like(score, dtype=int)
        
    #     # 对每列单独计算阈值
    #     for i in range(n_cols):
    #         col_score = score[:, i]
    #         std = np.std(col_score)
    #         mean = np.mean(col_score)
    #         threshold = mean + ratio * std
    ...
    #         # 生成该列的预测结果
    #         proba[:, i] = np.array(col_score > threshold, dtype=int)
...
def spot_eval(init_score, score, q=1e-3, level=0.02, depth=None):
    ...
    results = s.run()  # run
    proba = np.array(score>results['thresholds'], dtype=np.int32)
