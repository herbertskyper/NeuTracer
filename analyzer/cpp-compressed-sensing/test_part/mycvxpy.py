...
def reconstruct(n, d, index, value):
    ...
    ri = index
    b = x.T.flat[ri]
    # np.savetxt("b_py.csv", b)
    # np.savetxt("ri_py.csv", ri, fmt="%d")
    # b = value.T.flat
    ...
    # np.savetxt("transform_mat_py.csv", transform_mat, delimiter=",")
    ...
    # print("是否为 DPP？", prob.is_dcp(dpp=True))
    # prob.solve(solver='OSQP',cannon) #0.89
    # prob.solve(canon_backend=cvx.SCIPY_CANON_BACKEND)  #0.48
    prob.solve(solver='CLARABEL',verbose=False,enforce_dpp=True) #0.40
    # prob.solve(solver='OSQP')  # 0.40
    # data = prob.get_problem_data('CLARABEL')  # or 'OSQP', 'SCS', 'CLARABEL', etc.
    # print("CVXPY problem data:", data)
